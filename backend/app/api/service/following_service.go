...
func GetFollowing(currentUserID, targetUserID string) ([]map[string]interface{}, int, error) {
	...
	// 3) Check if user has permission to view following
	...
	following, err := model.GetFollowingList(targetUserID)
	if err != nil {
		return nil, http.StatusInternalServerError, errors.New("failed to retrieve following list")
	}

	return following, http.StatusOK, nil
}